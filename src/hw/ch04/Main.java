package hw.ch04;

import hw.ch04.framework.Factory;
import hw.ch04.framework.Product;
import hw.ch04.idcard.IDCardFactory;
import hw.ch04.license.LicenseCardFactory; 

public class Main {
    public static void main(String[] args) {
        System.out.println("=== IDCard Test ===");
        Factory factory1 = new IDCardFactory("2026-04-06");
        Product card1 = factory1.create("Kim Seo yeon");
        Product card2 = factory1.create("Kim Na yeon");
        card1.use();
        card2.use();

        System.out.println("\n=== LicenseCard Test ===");
        Factory factory2 = new LicenseCardFactory("2026-03-20");
        Product license1 = factory2.create("Park Jisung");
        Product license2 = factory2.create("Son Heung min");
        license1.use();
        license2.use();
    }
}